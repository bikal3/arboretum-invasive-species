...
/* Chart.js charts — skip gracefully if CDN failed to load */
if (typeof Chart === 'undefined') {
  console.warn('charts.js: Chart.js not loaded — charts skipped.');
} else {
  ...
  (function () {
    ...
    new Chart(document.getElementById('overallCoverageChart'), {
      ...
      data: {
        ...
        datasets: [{
          data: [stats.invasive_pct ?? 0, stats.non_invasive_pct ?? 0],
  ...
  (function () {
    (window.cardSpeciesData || []).forEach(s => {
      ...
      if (!canvas) {
        console.warn(`charts.js: canvas #densityBar_${s.code} not found`);
        return;
      }
      new Chart(canvas, {
        type: 'bar',
